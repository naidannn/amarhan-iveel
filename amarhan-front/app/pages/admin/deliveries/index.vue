...
const columns: Column<Delivery>[] = [
  { key: 'deliveryNumber', label: 'Дугаар', tabular: true, mobileTitle: true },
  { key: 'phone', label: 'Утас', tabular: true },
  { key: 'address', label: 'Хаяг' },
  { key: 'driver', label: 'Жолооч' },
  { key: 'status', label: 'Төлөв' },
  { key: 'packageCount', label: 'Ачаа', align: 'right', tabular: true },
  { key: 'unpaidBalance', label: 'Үлдэгдэл', align: 'right', tabular: true },
  { key: 'scheduledDate', label: 'Огноо', tabular: true },
]

/** routeMode-д зөвхөн «үүссэн» хүргэлт сонгогдож болно — бусад нь сонголтод утгагүй */
function isDeliverySelectable(d: Delivery) {
...
<template>
  <div>
    ...
    <UiDataTable
      ...
      :rows="rows"
      :loading="loading"
      :selectable="routeMode"
      :selected="selected"
      :is-row-selectable="isDeliverySelectable"
      empty-text="Хүргэлт олдсонгүй"
      @update:selected="selected = $event"
      @row-click="d => navigateTo(`/admin/deliveries/${d.id}`)"
    >
      ...
      <template #cell-deliveryNumber="{ row }">
        <span class="font-medium text-primary">{{ row.deliveryNumber }}</span>
      </template>

      <template #cell-address="{ row }">
        <span class="block max-w-[240px] truncate text-body-sm text-content-secondary">
          {{ row.address }}
        </span>
      </template>

      <template #cell-driver="{ row }">
        <span class="text-body-sm text-content-secondary">{{ driverLabel(row) }}</span>
      </template>

      <template #cell-status="{ row }">
        <UiStatusBadge :status="row.status" kind="delivery" size="sm" />
      </template>

      <template #cell-packageCount="{ row }">{{ packageCount(row) }}</template>

      <!-- §5.2 — аль хүргэлт блоклогдсоныг ЖАГСААЛТААС харна -->
      <template #cell-unpaidBalance="{ row }">
        <span :class="unpaidTotal(row) > 0 ? 'font-semibold text-error' : 'text-content-disabled'">
          {{ unpaidTotal(row) > 0 ? formatCurrency(unpaidTotal(row)) : '—' }}
        </span>
      </template>

      <template #cell-scheduledDate="{ row }">
        <span class="text-body-sm text-content-secondary">
          {{ formatDateTime(row.scheduledDate ?? row.createdAt) }}
        </span>
      </template>

      <template #empty>
        <UiBtn v-if="!activeFilterCount" class="mt-4" :icon="Plus" to="/admin/deliveries/new">
