# CLAUDE.md — Ивээл Карго систем

Энэ файл нь Claude Code (болон бусад AI туслах) энэ репод ажиллахад хэрэгтэй суурь мэдээллийг агуулна.
**Код бичихээс өмнө энэ файлыг, дараа нь холбогдох `docs/` файлыг унш.**

---

## 1. Төслийн мөн чанар

**Ивээл Карго** — Хятад (Эрээн) → Монгол чиглэлийн карго компанийн нэгдсэн удирдлагын систем.

> **Бүртгэл ХААНА хийгддэг (энэ нь бүх урсгалыг тодорхойлно):** Хятадаас ирдэг
> мэдээлэл нь **зөвхөн ачааны дугаар** (утасны дугаар ЗААВАЛ БИШ — ихэвчлэн
> ирдэггүй). Эрээнд ачааг систем дээр бүртгэдэггүй. **Бодит** бүртгэл (жин,
> эзлэхүүн, үнэ, байршил тодорхойлогдох мөч) ачаа **Монголд ирсний ДАРАА**
> хийгдэнэ. Үүнээс: хуучин гурван шатны замын төлөв (`in_transit`, `arrived`)
> **байхгүй**, үнийг бүртгэх үедээ тодорхойлно (жингээр тарифаар, эсвэл дүнг
> шууд бичих). `introduction.md` §1.1–1.5.
> **2026-07-31 (BR-45):** ажилтан бүртгэх мөчид сонгож болох нэг ЖИЖИГ
> УРЬДЧИЛСАН тэмдэглэл нэмэгдсэн — `in_erlian` төлөв: зөвхөн дугаараар
> мэдэгдэж байх үед бичигдэх ганц шат (дээрх зарчмыг зөрчихгүй — жин/үнэ/байршил
> хараахан байхгүй, байршлын нүд эзэлдэггүй, төлбөр/хүргэлтийн урсгалд
> ороогүй). Ачаа Монголд ирэхэд "Ирц бүртгэх"-ээр яг тэр бичлэг дээрээ
> `registered` болно. `docs/business-rules.md` BR-45, `roadmap.md` §2.19.
> **2026-08-01 (BR-45 update):** утас **ЗААВАЛ БИШ** болсон — зөвхөн
> `in_erlian` биш, `registered` ачаанд ч хамаарна; ачаа хэзээ ч харилцагчтай
> холбогдохгүй үлдэж болно. Төлбөр/хүргэлт/нэхэмжлэх ҮҮСГЭХЭД л харилцагч
> ЗААВАЛ шаардана (`ERROR_CODE.PHONE_REQUIRED`) — ажилтан `PUT /packages/:id`-
> ээр (`phone` талбар) дурын үед холбож болно. `docs/business-rules.md` BR-45.
> **2026-08-01 (BR-46):** ХАРИЛЦАГЧ өөрөө вэбээс ирэх ачаагаа урьдчилан
> мэдүүлдэг болов — `expected` төлөв (мөн л жин/үнэ/байршилгүй, нүд эзэлдэггүй).
> Ажилтан ТЭР дугаарыг бүртгэхэд систем ШИНЭ бичлэг үүсгэхгүй, яг тэр бичлэг
> дээр гүйцээнэ ("шингээх"). `docs/business-rules.md` BR-46, `roadmap.md` §2.20.
> **2026-08-03 (BR-21d):** Хүргэлт ХОЁР талаас дуусч болдог боллоо — ажилтан
> «Хүргэгдсэн» гэж тэмдэглэх (`delivered`) ЭСВЭЛ харилцагч өөрөө порталдаа
> «Хүргэлтээ авлаа» товч дарах (`received`). ХОЁР ТУСДАА терминал төлөв —
> нэгтгэвэл хэн баталгаажуулснаар ялгах боломж алдагдана. `received`-ыг
> ажилтны ерөнхий төлөв солих замаас ЗОРИУД хассан (зөвхөн харилцагчийн
> порталын endpoint). `docs/business-rules.md` BR-21d.
> **2026-08-17 (QPay, roadmap 5.6/5.7):** Эзэмшигч бодит production мерчант
> эрх өгсний дараа QPay онлайн төлбөрийг хэрэгжүүлэв — харилцагч хүргэлт
> захиалахдаа Данс/QPay-ийн аль нэгийг сонгоно. Шинэ `qpay.service.js`
> адаптер (`config.qpay.enabled`, `email.service.js`-ийн ижил зарчим).
> Webhook (`/v1/qpay/callback`, танилтгүй, тусдаа router) payload-д ХЭЗЭЭ Ч
> итгэхгүй — `qpayService.checkInvoice`-аар өөрийн эрхээр дахин
> баталгаажуулсан үр дүнд л мөнгө бичигдэнэ (QPay v2-д HMAC гарын үсэг
> байхгүй тул "поок → өөрийн эрхээр шалгах" загвар). Идемпотент боловсруулалт
> `paymentRepository.completeByProviderInvoice`-ийн атомик
> `findOneAndUpdate` (`status: pending` нөхцөл). `docs/business-rules.md`
> BR-21c (шинэчлэгдсэн), `roadmap.md` §5.6/5.7.
9 модулиас бүрдэнэ: Ачаа, Төлбөр, Хэрэглэгчийн вэб, Урамшуулал, Хүргэлт, Тайлан, Notification, Агуулах, Аюулгүй байдал.

Бүрэн бизнес шаардлага: [`introduction.md`](introduction.md) — **энэ бол эх сурвалж (source of truth)**.
Хөгжүүлэлтийн дараалал: [`roadmap.md`](roadmap.md).

### Одоогийн байдал (2026-08-01)

> **Урсгалын өөрчлөлт (2026-07-30):** Phase 2-ын дараа эзэмшигчийн тодруулгаар
> бүртгэлийн урсгал өөрчлөгдсөн — дэлгэрэнгүй `roadmap.md` §2.18.
> Салбарын seed код `ER` (Эрээн, Хятад) → `UB` (Улаанбаатар, Монгол) болов;
> `UB` нь **түр утга**, бодит агуулахын кодыг эзэмшигчээс лавлана.

> **Дэд бүтцийн хялбаршуулалт (2026-07-31):** Эзэмшигчийн шийдвэрээр MongoDB
> replica set, Docker, AWS S3 бүгд репогоос хасагдсан. Систем одоо локал
> **standalone** MongoDB руу шууд холбогддог, `withTransaction()` жинхэнэ Mongo
> транзакц ХИЙХГҮЙ болсон (дараалсан бичилт, rollback баталгаа алга —
> `docs/architecture.md` §4.3, §9 шийдвэр #2). Файл хадгалалт (AWS S3) хэрэгжээгүй
> байсан тул код цэвэрхэн устсан (`s3-upload.js`, `aws-sdk`, `multer-s3`, `s3-zip`).

**Phase 0–4 дууссан — MVP бэлэн** (ачаа бүртгэх → төлбөр авах → хүргэх өдөр
тутмын урсгал бүрэн ажиллана). **Phase 5 бараг бүрэн дууссан** — харилцагч
өөрөө бүртгүүлж, ачаа/төлбөрөө хянана, QPay-ээр онлайн төлнө (5.6–5.7,
2026-08-17). OTP (5.2) л үлдсэн.
**Phase 6-ийн Notification цөм хэсэг дууссан** (2026-08-01) — SMS (6.5) үлдсэн.
Бэлэн байгаа зүйл:

| Хэсэг | Байдал |
|---|---|
| Auth (login / me / logout / change-password) + JWT | ✅ |
| Салбар, агуулахын байршил, ачааны төрөл, тариф (хувилбаржсан) | ✅ |
| Харилцагч (утсаар автомат үүсэх) | ✅ |
| Audit Log (append-only) + харах UI | ✅ |
| **Ачааны модуль** — бүртгэл, төлөв (11, `expected`/`in_erlian` нэмэгдсэн), үнэ (2 горим), байршил, устгал/хүчингүй | ✅ |
| Хурдан бүртгэлийн UI, жагсаалт, дэлгэрэнгүй, баримт хэвлэх | ✅ |
| **"Эрээнд байгаа" урьдчилсан бүртгэл + Ирц бүртгэх** (BR-45, 2026-07-31) | ✅ |
| **Харилцагч өөрөө ачаа бүртгүүлэх + урьдчилсан бичлэг шингээх** (BR-46, 2026-08-01) | ✅ |
| Байршлын код бүртгэхэд СОНГОЛТООР (чөлөөт бичвэр хассан) | ✅ |
| Design System v1 + өөрсдийн компонентын сан (`components/ui/`) | ✅ |
| **Төлбөрийн модуль** — хуваасан төлбөр, нэгтгэсэн нэхэмжлэх, хуваарилалт, хүчингүй болгох | ✅ |
| Төлбөрийн жагсаалт, олон ачааг нэг дор төлүүлэх урсгал, нэхэмжлэх хэвлэх | ✅ |
| **Хүргэлтийн модуль** — §5.2 төлбөрийн хаалт, төлөв (6, `received` нэмэгдсэн), цуцлал, жолооч | ✅ |
| Хүргэлт үүсгэх урсгал, жагсаалт, өдрийн маршрут, маршрутын хуудас хэвлэх | ✅ |
| **Хүргэлт хоёр талаас дуусах** (BR-21d, 2026-08-03) — ажилтны «Хүргэгдсэн» ба харилцагчийн «Хүргэлтээ авлаа» тусдаа `delivered`/`received` төлөвт хадгалагдана | ✅ |
| **Харилцагчийн танилт** — утас+нууц үг, Google OAuth, `aud: 'customer'` токен | ✅ |
| **Харилцагчийн портал** — өөрийн ачаа, төлбөр, хүргэлт, профайл, хаяг | ✅ |
| Нээлттэй ачаа хайх (`/track`), Эрээний хаяг, FAQ + админаас засварлах | ✅ |
| **Харилцагч хүргэлт захиалах** (5.8, 2026-08-01) — ачаа+7000₮ хураамж, Данс сонговол `pending` төлбөр (ажилтан баталгаажуулна) эсвэл **QPay** (webhook автоматаар баталгаажуулна, 5.6/5.7, 2026-08-17) | ✅ |
| **Мэдэгдлийн модуль** (§7, roadmap Phase 6, 2026-08-01/02) — ачааны эвентээс автомат хувийн мэдэгдэл (BR-35: бүртгэгдсэн/ирсэн/төлбөр/хүргэлт), Админ/Менежерийн нийтийн зарлал (BR-36), нэвтрээгүй зочны мэдэгдлийн хуудас (`/notifications`, roadmap 6.3), зөвхөн `web` суваг | ✅ |
| **Утас баталгаажуулах (OTP)** — SMS үйлчилгээ сонгоогүй (roadmap 5.2) | ⛔ Phase 6 |
| **QPay онлайн төлбөр** (roadmap 5.6/5.7, 2026-08-17) — харилцагч хүргэлтийн хураамжаа QPay-ээр төлж болно, webhook автоматаар баталгаажуулна | ✅ |
| SMS суваг (roadmap 6.5) | ⛔ |
| **Зарлагын модуль + Өдрийн үр ашгийн тайлан** (BR-47, BR-47a, 2026-08-03) — байгууллагын ерөнхий зарлага (Менежер/Админ л бичнэ), `GET /v1/reports`-д "олох ёстой орлого" (Σ `finalPrice`, төлбөр орсон эсэхээс үл хамааран) vs өдрийн зарлагыг харьцуулсан `efficiency` хэсэг | ✅ |
| Урамшуулал (§4), Тайлан (§6) | ⛔ Phase 6+ |

Домэйны цөм логик `amarhan-api/src/domain/`-д DB-ээс хамааралгүй ЦЭВЭР функц
хэлбэрээр байна — шинэ дүрэм нэмэхдээ эндээс эхэлж, 100% тестээр хучина:

| Файл | Хариуцах дүрэм |
|---|---|
| `pricing.js` | BR-01, **BR-01a**, BR-02…BR-04 — жингийн шатлал, эзлэхүүн, гараар заасан үнэ, override хязгаар |
| `allocation.js` | BR-15…BR-17a — төлбөрийг ачаанууд дунд хуваарилах. `Σ allocations === amount` тогтмол |
| `package-state.js` | BR-07…BR-09a, BR-19, BR-45/BR-46 — төлөвийн шилжилт (замын шат БАЙХГҮЙ), урьдчилсан төлөв (`PRE_ARRIVAL`), байршил эзэмшил |
| `delivery-state.js` | BR-20…BR-21d — хүргэлтийн шилжилт, **§5.2 төлбөрийн хаалт** (override БАЙХГҮЙ), ачааны төлөв чирэх, харилцагчийн хүргэлтийн хураамжийн хаалт (`unpaidFee`, зөвхөн `createdBy: null`), хоёр тусдаа терминал төлөв `delivered`/`received` (BR-21d) |
| `tracking-number.js` | §1.3 — ачааны дугаарын нормчлол |
| `location-code.js` | BR-22 — байршлын кодын формат |
| `phone.js` | BR-27 — утасны нормчлол, лог маскчлал |

Phase 5-ын танилт ба хамрах хүрээний логик домэйнд БИШ, service давхаргад байна:

| Файл | Хариуцах зүйл |
|---|---|
| `services/customer-auth.service.js` | §3 — бүртгүүлэх/нэвтрэх/Google, утасны эрсдэлийн хязгаарлалт (дүрэм 13) |
| `services/customer-portal.service.js` | §3 — өөрийн өгөгдлийн хүрээ, цагаан жагсаалт (дүрэм 14) |
| `services/public.service.js` | §3 — нээлттэй хариунд юу гарахыг шийднэ (дүрэм 15) |
| `services/package.service.js` | BR-46 — `selfRegister`/`selfCancel` (харилцагчийн мэдүүлэг), `adoptExisting` (ажилтан урьдчилсан бичлэг дээр бүртгэх) |
| `services/delivery.service.js` | BR-21c (roadmap 5.8) — `selfCreate`/`deliverableForCustomer` (харилцагч өөрөө хүргэлт захиалах, эзэмшил шалгах); BR-21d — `selfConfirmReceived` (харилцагч өөрөө хүргэлтээ хүлээж авснаа баталгаажуулах) |
| `middlewares/authorize-customer.js` | `aud: 'customer'` танилт → `req.customer` |

---

## 2. Репогийн бүтэц

```
iveel-amarhan/
├── amarhan-api/          # Backend — Node.js + Express + MongoDB (Mongoose)
│   ├── src/
│   │   ├── config/       # env config, constants (роль, enum)
│   │   ├── models/       # Mongoose schema
│   │   ├── repositories/ # DB хандалт — зөвхөн энд Mongoose дуудна
│   │   ├── services/     # Бизнес логик
│   │   ├── controllers/  # HTTP req/res
│   │   ├── routes/api/   # Route тодорхойлолт
│   │   ├── validations/  # Joi схем
│   │   ├── middlewares/  # auth, validate, rate-limit, error-handler, sanitize
│   │   └── utils/        # logger, APIError, response
│   ├── scripts/          # seed / import скрипт
│   └── test/             # Mocha + Chai
├── amarhan-front/        # Frontend — Nuxt 4 + Vue 3 + Tailwind + Pinia
│   └── app/              # Nuxt 4-т бүх эх код `app/` дотор байна
│       ├── pages/  components/  composables/  stores/  layouts/  middleware/
├── docs/                 # Техникийн баримт бичиг (доор жагсаав)
├── introduction.md       # Бизнес шаардлага
└── roadmap.md            # Хөгжүүлэлтийн төлөвлөгөө
```

---

## 3. Аль docs-ыг хэзээ унших вэ

| Ажил хийхээс өмнө | Унших файл |
|---|---|
| Ямар нэг код бичих | [`docs/coding-style.md`](docs/coding-style.md) |
| Шинэ модуль/давхарга нэмэх | [`docs/architecture.md`](docs/architecture.md) |
| Шинэ Mongoose model / талбар нэмэх | [`docs/data-model.md`](docs/data-model.md) |
| Шинэ endpoint нэмэх | [`docs/api-conventions.md`](docs/api-conventions.md) |
| Ачаа/төлбөр/үнийн логик хөндөх | [`docs/business-rules.md`](docs/business-rules.md) |
| Эрх, audit log хөндөх | [`docs/security-and-permissions.md`](docs/security-and-permissions.md) |
| Тест бичих | [`docs/testing.md`](docs/testing.md) |
| Commit / PR хийх | [`docs/git-workflow.md`](docs/git-workflow.md) |
| Орчин тохируулах, deploy хийх | [`docs/setup-and-deployment.md`](docs/setup-and-deployment.md) |
| Монгол ↔ англи нэр томьёо | [`docs/glossary.md`](docs/glossary.md) |

---

## 4. Түгээмэл командууд

```bash
# Backend (amarhan-api/)
npm run dev            # nodemon-оор ажиллуулах (port 4000)
npm start              # production
npm run lint           # ESLint
npm run format         # Prettier
npm test               # Mocha (NODE_ENV=test)
npm run seed:admin     # Админ хэрэглэгч үүсгэх

# Frontend (amarhan-front/)
npm run dev            # Nuxt dev server (port 3000)
npm run build          # production build
npm run preview

# MongoDB — локал standalone mongod (Docker/replica set шаардлагагүй)
mongod                 # эсвэл системийн mongod сервис аль хэдийн ажиллаж байгаа бол алгасна
```

> Хоёулаа **npm** ашиглана (`package-lock.json`). Frontend-д `packageManager: yarn`
> гэсэн зөрчилтэй талбар байсныг Phase 0-д устгасан.

---

## 5. Заавал баримтлах дүрмүүд

Эдгээрийг зөрчсөн код review-д хүлээж авахгүй.

1. **Давхаргын дүрэм.** Controller → Service → Repository → Model. Controller дотор Mongoose
   дуудахыг, Repository дотор бизнес логик бичихийг хориглоно.
2. **Мөнгө = бүхэл тоо (₮).** Аравтын бутархай, float ашиглахгүй. Дэлгэрэнгүй: `docs/data-model.md`.
3. **Мөнгө/төлөв өөрчлөх бүр Audit Log-д бичигдэнэ.** Мартвал бизнес шаардлага зөрчигдөнө.
4. **Устгал бараг байхгүй.** Ачаа, төлбөрийг `cancelled`/`voided` төлөвт шилжүүлнэ.
   Ганц үл хамаарах тохиолдол: `introduction.md` §1.6 (Админ, 24ц дотор, төлбөргүй).
5. **Хайлт бүр индекслэгдсэн байх, server талд хийгдэх, хуудаслагдсан байна.** Client талд
   бүх өгөгдлийг татаж filter хийхийг хориглоно (§9.3).
6. **Эрхийн шалгалт route бүрт `authorize(...)`-оор хийгдэнэ.** UI-д товч нуух нь хамгаалалт биш.
7. **Хатуу кодлосон нууц үг/түлхүүр байхгүй.** Бүгд `.env`-ээс. *(Одоо `src/services/qpay.js`-д
   өөр төслийн QPay мерчант эрх хатуу бичигдсэн байгаа — Phase 0-д заавал арилгана.)*
8. **UI текст монгол хэлээр, код (хувьсагч, enum, коллекц) англи хэлээр.** Заримыг холихгүй.
   Харгалзуулалт: `docs/glossary.md`.
9. **Ачаа бүртгэх урсгалыг удаашруулж болохгүй.** §1.4 — хуудас reload хийхгүй, дараалсан
   бүртгэлийг тасалдуулахгүй.
10. **Мөнгийг ЗӨВХӨН `payment.service.js` бичнэ.** `packages.paidAmount`, `balance`,
   `paymentStatus`, `invoices.paidAmount` — эдгээрийг өөр хаана ч бичихийг хориглоно
   (BR-14). Үлдэгдлийг ХАСАХ бус эх сурвалжаас ДАХИН БОДНО — хасалт нь зөрүүг
   үүрд үлдээдэг. `Σ allocations === amount` тогтмол хэзээ ч зөрч болохгүй (BR-17).
11. **§5.2-ын хаалтад ҮЛ ХАМААРАХ ТОХИОЛДОЛ БАЙХГҮЙ.** Төлбөр дутуу ачааг
   хүргэлтэнд гаргах нь Админд ч хоригтой (BR-19, BR-20). `delivery-state.js`-д
   `system` / `force` / `skipPaymentCheck` гэсэн параметр нэмэхийг хориглоно —
   сонголт байхгүй бол тойрох зам ч байхгүй. Зээлээр хүргэх нь тусдаа функц
   (`introduction.md` §5.2, v2).
12. **Үнийн хоёр горимыг хоёуланг хүндэтгэ.** Жин оруулаагүй бүртгэл нь алдаа БИШ —
   `priceSource: 'manual'`, `pricingSnapshot: null` нь зөв, хүчинтэй бүртгэл (BR-01a).
   Тариф хэрэглээгүй ачаанд **хуурамч snapshot хадгалахыг хориглоно** — хожим жин
   нэмэгдэхэд буруу дүн бодогдоно. `manual` ачаанд ±20%-ийн хязгаар хамаарахгүй.
13. **Харилцагч утсаа ӨӨРӨӨ солихыг хориглоно** (Phase 5). Утас бол ачааг холбох гол
   түлхүүр (BR-26): бүртгүүлэхэд тэр дугаарын БҮХ ачаа харагдана. OTP хараахан
   байхгүй (roadmap 5.2) тул утас солих зам нээвэл нэг бүртгэлээр дараалан утас
   сольж бүх хүний ачааг харна. `customer-auth.validation.js`-ийн `updateProfile`-д
   `phone` талбар нэмэхийг хориглоно. Мөн **`phoneVerified`-ыг бүртгэлийн үед
   `true` болгож болохгүй** — баталгаажаагүйг баталгаажсан гэж бичих нь Phase 6-д
   OTP асаахад аль бичлэг бодитоор баталгаажсаныг ялгах боломжгүй болгоно.
14. **Харилцагчийн API-д хамрах хүрээ ҮРГЭЛЖ токеноос гарна.** `req.customer._id`-г
   л ашиглана; клиентээс ирсэн `customerId`/`phone`-г ХЭЗЭЭ Ч авахгүй.
   `{ ...req.query }`-г repository руу дамжуулахыг хориглоно — хожим шүүлт
   нэмэгдэхэд хамрах хүрээ чимээгүй нээгдэнэ. Эрхгүй бичлэгт `403` биш **`404`**
   (403 нь тухайн ID оршин байгааг батална). Хариу нь **цагаан жагсаалттай**:
   `note`, `locationCode`, `registeredBy`, `pricingSnapshot` зэрэг дотоод талбар
   харилцагчид гарахгүй (`customer-portal.service.js`).
15. **`/v1/public/*`-д нэмсэн зам БҮР интернэтэд ил.** Нээлттэй хариунд үнэ, үлдэгдэл,
   бүтэн утас, агуулахын байршил оруулахыг хориглоно (`public.service.js`).
   Статик агуулгыг угтвараар БИШ, `PUBLIC_CONTENT_KEYS` **жагсаалтаар** зөвшөөрнө —
   угтварын шүүлт нь хожим нэмэгдэх `content.internal_*` төрлийн түлхүүрийг
   чимээгүй нийтэлнэ.

---

## 6. Хийхээс өмнө асуух ёстой зүйл

Дараах тохиолдолд өөрөө шийдэлгүй, хэрэглэгчээс асуу:

- Тарифын бодит тоон утга, урамшууллын босго (эдгээр нь бизнесийн шийдвэр)
- Одоо байгаа `users` коллекцын өгөгдөлд нөлөөлөх migration
- QPay / SMS үйлчилгээ үзүүлэгчийн бодит гэрээ, эрхийн мэдээлэл
- `introduction.md`-д тодорхойгүй үлдсэн бизнес дүрэм

---

## 7. Phase 0 цэвэрлэгээ — дууссан (2026-07-30)

Boilerplate-ийн хог код устаж, аюулгүй байдлын нүхнүүд таглагдсан. Дэлгэрэнгүйг
git түүхээс (`chore/p0-cleanup` салаа) харна уу. Гол өөрчлөлтүүд:

| Юу | Үр дүн |
|---|---|
| Ролийн бүтэц | `admin` / `manager` / `staff` (`senior_manager` арилсан) |
| Нээлттэй `POST /auth/register` | **Хаагдсан** — эрх өөрөө өсгөх нүх байсан. Ажилтныг зөвхөн Админ `POST /api/v1/users`-ээр үүсгэнэ |
| JWT | 8 цагийн хугацаа + `aud: 'staff'` нэмэгдсэн (өмнө нь хугацаагүй байсан) |
| Идэвхгүй ажилтан | Хүчинтэй токентой ч хаагдана |
| Хатуу кодлосон нууц | qpay.js, slack.js устсан; s3-upload.js-ийнх арилсан; `demoLogin()` устсан |
| MongoDB | docker-compose-д replica set (`rs0`) автоматаар эхэлнэ *(2026-07-31: буцаагдсан, доор үзнэ үү)* |
| Транзакц | `src/utils/transaction.js` → `withTransaction()` *(2026-07-31: session/rollback-гүй болсон)* |
| Тест | mongodb-memory-server (replica set), 18 тест өнгөрдөг *(2026-07-31: standalone болсон)* |
| Lint | ESLint 9 flat config, Prettier-тэй зөрчилдөхгүй |
| CI | `.github/workflows/ci.yml` — lint, format, test, build, нууц түлхүүр шалгах |

**Хараахан ажиллуулаагүй:** `scripts/migrations/001-align-staff-roles.js` — одоо байгаа
`users` өгөгдлийн ролийг хөрвүүлнэ. Backup хийж, `npm run migrate -- --dry` шалгасны
дараа ажиллуулна.

> ⚠️ **Алдагдсан эрхийг солих шаардлагатай** — эдгээр репод хатуу кодлогдсон байсан:
> AWS IAM түлхүүр (`s3-upload.js`, файл өөрөө 2026-07-31-нд устсан ч гэсэн УРЬД
> commit хийгдсэн IAM түлхүүрийг солих шаардлага хэвээр байна), Slack bot token
> (`slack.js`), QPay мерчант эрх (`qpay.js`), SSH private key (`naidan-main.pem`,
> одоо `~/.ssh/`-д). Утгыг нь энд бичихгүй — солих ажлыг гүйцэтгэсэн эсэхийг
> эзэмшигчээс лавлана.

### 7.1 Дэд бүтцийн буцаалт (2026-07-31)

Эзэмшигчийн шийдвэрээр Phase 0-ийн дараах шийдвэрүүдийг буцаасан:

| Юу | Хуучин | Шинэ |
|---|---|---|
| MongoDB | Docker + replica set (`rs0`) | Локал **standalone** mongod, `.env`-ийн `MONGOURI` шууд заана |
| Транзакц | `withTransaction()` жинхэнэ Mongo session/rollback-той | `withTransaction()` зөвхөн callback-аа дараалан ажиллуулна — **rollback байхгүй** |
| Docker | `docker-compose.yml`, `amarhan-api/Dockerfile` | Устсан — dev ажиллуулахад Docker шаардахгүй |
| Файл хадгалалт (AWS S3) | `s3-upload.js`, `aws-sdk`, `multer-s3`, `s3-zip` (код бэлэн ч route холбогдоогүй байсан) | Устсан |
| Тест | `mongodb-memory-server` replica set горимд | `mongodb-memory-server` standalone горимд (илүү хурдан) |

**Trade-off:** мөнгө/audit өөрчлөлтийн **атомик rollback баталгаа алдагдсан**
(`docs/architecture.md` §9 шийдвэр #2, `docs/business-rules.md` BR-41). Хэрэв
дараа нь атомик баталгаа дахин хэрэгтэй болвол replica set-ийг буцааж тохируулах
хэрэгтэй (docker-compose-гүйгээр ч болно — `mongod --replSet rs0`).

> **Илрүүлсэн регресс, засагдсан:** энэ өөрчлөлт хийхэд `payment.test.js`-ийн
> "зэрэг орсон хоёр төлбөр хамтдаа хэтрэвэл нэг нь бүтэлгүйтнэ" (BR-15) тест
> унасан — жинхэнэ транзакцын write-conflict илрүүлэлт байхгүй болсноор зэрэг
> ирсэн хоёр төлбөр хамтдаа үлдэгдлээс хэтрэх боломжтой болсон байсан. Үүнийг
> MongoDB-ийн **нэг баримт бичгийн атомик** `findOneAndUpdate` ашигласан
> `reserveAllocations`-аар (`payment.service.js`, `package.repository.js`)
> засварласан — эдгээр нь replica set шаардахгүй. Тест дахин ногоон.

---

## 8. Хариултын хэл

Хэрэглэгчтэй **монгол хэлээр** харилц. Код доторх comment мөн монгол хэлээр бичиж болно
(одоо байгаа кодод англи comment зонхилдог — файл дотроо нэг хэвийг бариарай).
